...
# Métricas de análise de redes sociais, implementadas do zero para controle total
@dataclass
...
# Índice de grafo pré-computado para acelerar cálculos de métricas que exigiriam varreduras O(n) repetidas.
@dataclass
class _GraphIndex:
    ...
    # Construção a partir de um grafo genérico, com otimizações para tipos específicos.
    @classmethod
    ...
    # Construção otimizada para grafos de lista de adjacência e matriz de adjacência, evitando chamadas genéricas.
    @staticmethod
...
# BFS para calcular distâncias, usado em closeness centrality. Pode ser reutilizado para outras métricas que exigem distâncias.
...
# Implementações de métricas, todas usando o _GraphIndex para acesso eficiente às adjacências e graus. As implementações são feitas do zero para controle total sobre a complexidade e otimizações específicas.
...
# Betweenness centrality usando o algoritmo de Brandes
...
# Closeness centrality baseada na soma das distâncias a todos os outros vértices alcançáveis. 
...
# Eigenvector centrality via power iteration em A², onde A é a adjacência simetrizada. 
...
# PageRank usando power iteration, com tratamento de nós sem saída (dangling nodes) e normalização final.
def pagerank(
...
# Densidade do grafo: número de arestas dividido pelo número máximo possível de arestas (n*(n-1) para grafos dirigidos sem loops).
...
# Coeficiente de clustering global: razão entre o número de triângulos e o número de tríades (pares de vizinhos) no grafo. Implementação O(n * d^2) onde d é o grau médio, usando conjuntos para verificar conexões entre vizinhos.
...
# Assortatividade de grau: correlação de Pearson entre os graus dos extremos das arestas. 
...
# Detecção de comunidades via label propagation: cada vértice adota o rótulo mais comum entre seus vizinhos, iterativamente, até convergência. O resultado é um dicionário de vértice para comunidade (rótulo).
...
# Modularity: medida da qualidade da divisão em comunidades, comparando a densidade de arestas dentro das comunidades com a esperada por acaso. Implementação O(m) onde m é o número de arestas, usando os graus pré-computados para calcular a contribuição de cada aresta.
...
# Bridging ties: identifica os vértices que conectam comunidades diferentes, usando a centralidade de intermediação para priorizar os mais importantes. Retorna os top_k vértices com maior betweenness centrality que têm vizinhos em pelo menos 2 comunidades diferentes.
def bridging_ties(
    idx: _GraphIndex,
    communities: Dict[int, int],
...
# Função principal para calcular todas as métricas, com opções de verbose e callback de progresso para feedback durante cálculos demorados. As métricas são calculadas em uma ordem que permite reutilizar índices pré-computados e evitar cálculos redundantes.
def compute_metrics(
    g: AbstractGraph,
    *,
    verbose: bool = False,
